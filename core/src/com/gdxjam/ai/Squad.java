...
public class Squad {
	...
	public void addMember(Entity entity) {
		SteerableBodyComponent steerable = Components.STEERABLE_BODY
				.get(entity);
		SteeringBehaviorComponent behaviorComponent = Components.STEERING_BEHAVIOR
				.get(entity);
		members.add(entity);
		agents.add(steerable);

		RadiusProximity<Vector2> cohesionProximity = new RadiusProximity<Vector2>(
				steerable, agents, 8f);
		RadiusProximity<Vector2> seperationProximity = new RadiusProximity<Vector2>(
				steerable, agents, 2f);

		Cohesion<Vector2> cohesion = new Cohesion<Vector2>(steerable,
				cohesionProximity);
		Separation<Vector2> separation = new Separation<Vector2>(steerable,
				seperationProximity);
		Alignment<Vector2> align = new Alignment<Vector2>(steerable,
				cohesionProximity);
		Arrive<Vector2> arrive = new Arrive<Vector2>(steerable, target)
				.setTimeToTarget(0.01f).setArrivalTolerance(0.002f)
				.setDecelerationRadius(5f);

		BlendedSteering<Vector2> behavior = new BlendedSteering<Vector2>(
				steerable);
