 ...
 * Generates the game world by creating an asteroid field using fBm applied OpenSimplexNoise
...
public class WorldGenerator {
	...
	/**
	 * Evaluates a x,y pair with fBm applied OpenSimplexNoise.
	 * Applies a radial mask to the value.
	 * @param x The x coord
	 * @param y The y coord
			...
			for(int col = 0; col < totalCols; col++){
				...
				if(heightValue <=  param.heightThreshold){
					Vector2 pos = new Vector2(
						(row * rowSpacing) + (MathUtils.random(-param.asteroidScattering, param.asteroidScattering) * rowSpacing),
						(col * colSpacing) + (MathUtils.random(-param.asteroidScattering, param.asteroidScattering) * colSpacing));
